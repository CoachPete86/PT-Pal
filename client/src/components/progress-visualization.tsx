...
export function ProgressVisualization({
  ...
  const processedWorkoutData =
    progressData?.workouts?.map((entry: any) => ({
  ...
  function calculateStrengthScore(exercises: any[] = []) {
    ...
    const strengthExercises = exercises.filter(
      (ex) => ex.type === "strength" && ex.weight && ex.reps,
    );

    if (strengthExercises.length === 0) return 0;

    return (
      strengthExercises.reduce((sum, ex) => sum + ex.weight * ex.reps, 0) /
  ...
  function calculateEnduranceScore(exercises: any[] = []) {
    ...
    const cardioExercises = exercises.filter(
      (ex) => ex.type === "cardio" && ex.duration,
    );

    if (cardioExercises.length === 0) return 0;

    return (
      cardioExercises.reduce((sum, ex) => sum + ex.duration, 0) /
